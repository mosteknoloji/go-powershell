...
func (c *UserPasswordCredential) prepare(s Middleware) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("Could not convert password to secure string: %+v", err))
	}
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("Could not create PSCredential object: %+v", err))
	}
